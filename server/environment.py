...
class SQLRepairEnvironment(Environment):
    ...
    def __init__(self):
        ...
        # Fault injection
        self._fault_injector = FaultInjector(difficulty_level=2)
        self._injected_faults: List[str] = []
        self._episode_seed: Optional[int] = None
        self._injected_hints: List[str] = []
    ...
    def _reset_unlocked(self, task_id: str = "easy", seed: Optional[int] = None) -> SQLRepairObservation:
        ...
        injection_result = self._fault_injector.inject(
            conn=self._conn,
            task_id=task_id,
            broken_query=task["broken_query"],
            seed=seed,
        )
        injected_broken_query = injection_result["broken_query"]
        self._injected_faults = injection_result["injected_faults"]
        self._injected_hints = self._fault_injector.get_hints(self._injected_faults)
        ...
        self._state = SQLRepairState(
            ...
            task_id=task_id,
            step_count=0,
            injected_fault_count=len(self._injected_faults),
            difficulty_level=self._fault_injector.difficulty_level,
        )
        ...
        return SQLRepairObservation(
            ...
            rows_affected=0,
            partial_score=0.0,
            hint=f"TASK ({task_id.upper()}): {task['description']}\n\nBROKEN QUERY:\n{injected_broken_query}",
            step_count=0,
            tables=[],
            task_id=task_id,
            task_description=task["description"],
            broken_query=injected_broken_query,
        )
    ...
    def _step_unlocked(self, action: SQLRepairAction) -> SQLRepairObservation:
        ...
        # ── progressive hints (using injected fault-specific hints) ───────
        hint = obs.hint
        if not hint and current_potential == self._prev_potential:
            if step == 5 and self._hint_index == 0:
                hint = self._injected_hints[0] if self._injected_hints else ""
                self._hint_index = 1
            elif step == 10 and self._hint_index <= 1:
                hint = self._injected_hints[1] if len(self._injected_hints) > 1 else ""
                self._hint_index = 2
            elif step == 15 and self._hint_index <= 2:
                hint = self._injected_hints[2] if len(self._injected_hints) > 2 else ""
                self._hint_index = 3
    ...
    def set_difficulty_level(self, level: int):
        """Update fault injector difficulty level (0-3). Takes effect on next reset()."""
        self._fault_injector.difficulty_level = min(max(level, 0), 3)
